...
- **非文本消息限制**（可选，群主可通过 `/activity` 控制）：
  - 启用时：活跃度 ≤ 0 的用户无法发送图片、贴纸、视频等富媒体，以及联系人 / 投票 / 位置 / 清单 / 故事 / 骰子等结构化消息
  - 禁用时：新用户也可自由发送非文本消息
- **活跃度规则**：
  - 普通文本消息：+1 活跃度
...
- **按群组/用户切换**：`/lang [locale]` 命令切换语言偏好（群组语言 + 用户私聊语言独立设置；支持 `/lang en` 直接切换，匿名管理员也可用）
...
- `/start` - 查看帮助信息
- `/help` - 查看帮助信息
- `/lang [locale]` - 切换语言（简体中文 / 繁體中文 / English）
...
- [x] **v1.12.x**: AI 检测新增 TypeSafe Jev 决策模型协议 `typesafe_systemone`（不生成文本、直接返回垃圾概率与类别，约 500ms，兼容 TypeSafe 官方与 OpenRouter 端点，可作文本主服务商、LLM 作备份）；群消息紧凑化（无空行分段、字段同行）、动画贴纸多帧合并为一次 Vision 请求、修复跨聊天回复防护因 `origin.type` 取值报错而整体失效
