...
#### 跨聊天回复引流防护（Reply in Another Chat）🔗
- **攻击手法**：已入群假人用「Reply in Another Chat」发消息，正文无意义，广告承载在指向外部频道消息的引用预览里；Bot API 不下发被引用原文，内容检测不可达
- **检测方式**：纯结构信号——消息带 `external_reply` 即命中，所有 origin 类型（频道 / 用户 / 隐藏用户 / 群）统一对待；豁免论坛群跨 topic 回复与本群关联频道的引用（`linked_chat_id` 走 Redis 缓存 600s）
- **处置**：命中生成合成检测结果（confidence 0.9）交既有路由——确认模式群进入人工复核 / 集体投票，其余群删除 + 处罚；检测点位于消息前置检查第 8 步（管理员豁免之后），一次覆盖全部媒体类型与编辑消息，不受活跃度豁免影响
- **群开关**：`anti_external_reply_enabled`（默认启用，/groupset 子菜单可关）
- **训练样本**：sample_text 回退链 正文 → caption → 引用文本截 200；`add_feedback` 增加空文本守卫，空文档不再入库
- **数据库迁移** `e8d8f4f6ed3e`：groups 表新增 `anti_external_reply_enabled`（boolean 默认 true）

### Bug 修复

#### AI 正样本入库延后到置信度调整之后 🧪
- AI 判垃圾的消息原本在结果合并阶段即入库为正样本，早于活跃度调整与上下文一致性调整；被改判为正常放行的消息已进入垃圾训练集，污染后续重训
- 现统一收口到 `_collect_ai_training_sample`，在全部调整完成、最终仍判垃圾、且合并采用的是 AI 结果时才入库；负样本语义不变；确认模式与异常降级路径不入库；入库失败不影响检测结果

#### 群组上下文完整送给 AI 🧵
- `format_context_for_ai` 原先把缓存倒序后再截取前 5 条，缓存满 `CONTEXT_MESSAGE_COUNT`（默认 10）条时交给 AI 的是**较旧**的 5 条而非最近对话；现按时间正序输出全部缓存消息（排除当前消息），不再二次截断

#### 训练样本同文本去重，人工标注优先 🧹
- **背景**：同一条消息会经多个入库入口各写一行同标签样本（AI 自动标注 → bot 立即处罚 → 管理员反馈 / `/spam` / `/notspam` / 举报批准 / 投票终局），训练时重复计权；「查旧正样本 → 删一行 → 新增」的纠错逻辑散落三处且只删单行，历史重复行仍参与训练
- **新规则**（`SpamRepository.upsert_sample`，全部业务入口统一走它）：按精确文本锁全部同文本行，无则插入；人工标注覆盖最新行并删除其余重复行（管理员决定始终生效）；自动标注（AI 保留 ID / bot ID，由配置推导）遇任一人工标注即跳过，否则覆盖并收敛；覆盖时刷新 `created_at`，纠正后的样本参与下一轮训练
- `/notspam` 与立即处罚后的反馈按钮不再先删后加；`add_feedback` 被人工标注保护跳过时返回 False，不触发自动训练
- 负样本裁剪的 DELETE 阶段追加 `(created_at, id)` 时间边界复核，防止 victim 选取后被人工纠正刷新时间的负样本仍被删；`upsert_sample` 对 PostgreSQL 死锁（与裁剪交错）整事务重试
- **已知限制**：无唯一约束，并发首次写入仍可能各插一行，下次同文本写入时收敛

### 代码质量

- `_merge_detection_results` 改为纯合并（签名去掉 text / skip_auto_train）；样本入库前快照传统判定，避免上下文调整原地修改结果后无法区分来源
- `SpamSample` 模型 `confidence` / `labeled_by` 注解改为 Optional（列本就 nullable，无迁移）
- 测试 +76：跨聊天回复 21（四种 origin / 豁免矩阵 / 缓存 / 兜底链 / 异常吞咽）、入库时机 22（活跃度 / 上下文改判、来源快照、确认模式、降级路径、上下文顺序）、样本去重 33（来源判定、覆盖 / 收敛 / 保护矩阵、端到端序列、死锁重试、裁剪边界复核）
